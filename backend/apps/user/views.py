...
class DemoteFromAuxiliarView(APIView):
    permission_classes = (IsAuthenticated,)

    @swagger_auto_schema(
        operation_description="Remover a un auxiliar de una salida a campo específica. Solo disponible para profesores.",
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
                "fieldtrip_id": openapi.Schema(type=openapi.TYPE_INTEGER, description="ID de la salida a campo.")
        ...
        responses={
            200: openapi.Response("Usuario removido como auxiliar exitosamente."),
            400: "El usuario no es un auxiliar en esta salida.",
            403: "No tienes permiso para realizar esta acción.",
            404: "Usuario o salida a campo no encontrado."
        }
    )
    def post(self, request, pk, *args, **kwargs):
        if request.user.role != 'teacher':
            return Response(
                {"error": "Solo los profesores pueden remover auxiliares."},
                status=status.HTTP_403_FORBIDDEN,
            )
        try:
            user = User.objects.get(pk=pk)
        ...
        try:
            fieldtrip = Fieldtrip.objects.get(pk=fieldtrip_id)
            try:
                attendee = FieldtripAttendee.objects.get(
                    user=user,
                    fieldtrip=fieldtrip
                )
                ...
                attendee.save()
                return Response(
                    {"message": f"{user.names} {user.surnames} ha sido removido como auxiliar de {fieldtrip.name}."},
                    status=status.HTTP_200_OK,
                )
            except FieldtripAttendee.DoesNotExist:
                return Response(
                    {"error": "El usuario no está registrado en esta salida a campo."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except Fieldtrip.DoesNotExist:
            return Response(
...
class PromoteToGroupLeaderView(APIView):
    permission_classes = (IsAuthenticated,)

    @swagger_auto_schema(
        operation_description="Marcar a un estudiante como líder de grupo en una salida a campo específica. Solo disponible para profesores.",
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
                "fieldtrip_id": openapi.Schema(type=openapi.TYPE_INTEGER, description="ID de la salida a campo.")
        ...
        responses={
            200: openapi.Response("Usuario marcado como líder de grupo exitosamente."),
            400: "El usuario no es un estudiante o no está registrado en la salida.",
            403: "No tienes permiso para realizar esta acción.",
            404: "Usuario o salida a campo no encontrado."
        }
    ...
    def post(self, request, pk, *args, **kwargs):
        if request.user.role != 'teacher':
            return Response(
                {"error": "Solo los profesores pueden marcar líderes de grupo."},
                status=status.HTTP_403_FORBIDDEN,
            )
        try:
            user = User.objects.get(pk=pk)
...
class DemoteFromGroupLeaderView(APIView):
    permission_classes = (IsAuthenticated,)

    @swagger_auto_schema(
        operation_description="Quitar a un estudiante el rol de líder de grupo en una salida a campo específica. Solo disponible para profesores.",
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
                "fieldtrip_id": openapi.Schema(type=openapi.TYPE_INTEGER, description="ID de la salida a campo.")
        ...
        responses={
            200: openapi.Response("Usuario removido como líder de grupo exitosamente."),
            400: "El usuario no es un líder de grupo en esta salida.",
            403: "No tienes permiso para realizar esta acción.",
            404: "Usuario o salida a campo no encontrado."
        }
    )
    def post(self, request, pk, *args, **kwargs):
        if request.user.role != 'teacher':
            return Response(
                {"error": "Solo los profesores pueden remover líderes de grupo."},
                status=status.HTTP_403_FORBIDDEN,
            )
        try:
            user = User.objects.get(pk=pk)
        ...
        try:
            ...
            try:
                ...
                attendee.save()
                return Response(
                    {"message": f"{user.names} {user.surnames} ya no es líder de grupo en {fieldtrip.name}."},
                    status=status.HTTP_200_OK,
                )
            except FieldtripAttendee.DoesNotExist:
                return Response(
